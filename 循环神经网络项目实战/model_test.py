...
# --------------------------------- 4. 数据归一化 ---------------------------------
# 创建一个MinMaxScaler实例，将数据缩放到0到1的范围
# 注意：这里我们应该使用在训练阶段已经fit过的scaler，或者重新fit测试数据。
# 为了独立测试，这里我们重新fit测试数据，但在实际应用中，应该保存并加载训练时的scaler。
sc = MinMaxScaler(feature_range=(0, 1))
test_set_scaled = sc.fit_transform(test_set_original)
...
real_prices = test_set_original.values[5:]
